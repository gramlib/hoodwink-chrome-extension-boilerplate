...
import { CancellationToken, CancellationTokenSource } from '@hoodwink/common/base/cancellation';
import * as errors from '@hoodwink/common/base/errors';
import { Emitter, Event } from '@hoodwink/common/base/event';
import { IDisposable, toDisposable } from '@hoodwink/common/base/lifecycle';
...
export async function raceCancellablePromises<T>(
  cancellablePromises: Array<CancelablePromise<T>>,
): Promise<T> {
...
export function sequence<T>(promiseFactories: Array<ITask<Promise<T>>>): Promise<T[]> {
  const results: T[] = [];
  let index = 0;
...
export class Limiter<T> {
  ...
  private outstandingPromises: Array<ILimitedTaskFactory<T>>;
