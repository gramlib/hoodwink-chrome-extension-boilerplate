...
import { matchesFuzzy, IMatch } from '@hoodwink/common/base/filters';
import { ltrim } from '@hoodwink/common/base/strings';
...
export function matchesFuzzyCodiconAware(
  query: string,
  target: IParsedCodicons,
  ...
  const matches = matchesFuzzy(
    query,
    wordToMatchAgainstWithoutCodiconsTrimmed,
  ...
  if (matches) {
    for (const match of matches) {
      const codiconOffset =
        codiconOffsets[match.start + leadingWhitespaceOffset] /* codicon offsets at index */ +
