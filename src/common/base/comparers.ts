...
import { IdleValue } from '@hoodwink/common/base/async';
...
const intlFileNameCollatorNumericCaseInsenstive: IdleValue<{
  collator: Intl.Collator;
}> = new IdleValue(() => {
...
export function noIntlCompareFileNames(
  one: string | null,
  other: string | null,
  caseSensitive = false,
): number {
...
export function compareFileExtensionsDefault(one: string | null, other: string | null): number {
  ...
  result = compareAndDisambiguateByLength(
...
function extractNameAndExtension(str?: string | null, dotfilesAsNames = false): [string, string] {
  const match = str ? (FileNameMatch.exec(str) as string[]) : ([] as string[]);
