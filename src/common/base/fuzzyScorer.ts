...
import { compareAnything } from '@hoodwink/common/base/comparers';
import {
  matchesPrefix,
  IMatch,
  isUpper,
...
} from '@hoodwink/common/base/filters';
import { sep } from '@hoodwink/common/base/path';
import { isWindows, isLinux } from '@hoodwink/common/base/platform';
import { stripWildcards, equalsIgnoreCase } from '@hoodwink/common/base/strings';
...
export function scoreItemFuzzy<T>(
  ...
  if (description) {
    cacheHash = `${label}${description}${query.normalized}${
      Array.isArray(query.values) ? query.values.length : ''
    }${fuzzy}`;
  } else {
    cacheHash = `${label}${query.normalized}${
      Array.isArray(query.values) ? query.values.length : ''
    }${fuzzy}`;
  }
...
function normalizeQuery(
  original: string,
): { pathNormalized: string; normalized: string; normalizedLowercase: string } {
