...
import * as extpath from '@hoodwink/common/base/extpath';
import * as paths from '@hoodwink/common/base/path';
import { LRUCache } from '@hoodwink/common/base/map';
import { CharCode } from '@hoodwink/common/base/charCode';
import { isThenable } from '@hoodwink/common/base/async';
...
interface ParsedStringPattern {
  (path: string, basename?: string):
    | string
    | null
    | Promise<string | null> /* the matching pattern */;
  basenames?: string[];
  patterns?: string[];
  allBasenames?: string[];
  allPaths?: string[];
...
function trivia4and5(path: string, pattern: string, matchPathEnds: boolean): ParsedStringPattern {
  const nativePath =
    paths.sep !== paths.posix.sep ? path.replace(ALL_FORWARD_SLASHES, paths.sep) : path;
  const nativePathEnd = paths.sep + nativePath;
  const parsedPattern: ParsedStringPattern = matchPathEnds
    ? function (path, basename) {
        return typeof path === 'string' && (path === nativePath || path.endsWith(nativePathEnd))
          ? pattern
          : null;
      }
    : function (path, basename) {
...
function parseExpressionPattern(
  ...
  if (value) {
    const { when } = <SiblingClause>value;
...
function aggregateBasenameMatches(
  ...
  const basenames = basenamePatterns.reduce<string[]>((all, current) => {
    const { basenames } = <ParsedStringPattern>current;
  ...
  if (result) {
    ...
    patterns = basenamePatterns.reduce((all, current) => {
      const { patterns } = <ParsedStringPattern>current;
