...
import {
  ...
  Keybinding,
  SimpleKeybinding,
} from '@hoodwink/common/base/keyCodes';
import { OperatingSystem } from '@hoodwink/common/base/platform';
import { ScanCodeBinding, ScanCodeUtils } from '@hoodwink/common/base/scanCode';
...
export class KeybindingParser {
  ...
  private static parseSimpleUserBinding(
    ...
    if (scanCodeMatch) {
      ...
      return [
        new ScanCodeBinding(mods.ctrl, mods.shift, mods.alt, mods.meta, scanCode),
        mods.remains,
      ];
    }
    const keyCode = KeyCodeUtils.fromUserSettings(mods.key);
  ...
  static parseUserBinding(input: string): Array<SimpleKeybinding | ScanCodeBinding> {
    ...
    const parts: Array<SimpleKeybinding | ScanCodeBinding> = [];
