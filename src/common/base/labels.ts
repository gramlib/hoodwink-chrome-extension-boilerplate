...
import { posix, normalize, win32, sep } from '@hoodwink/common/base/path';
import { startsWithIgnoreCase, rtrim } from '@hoodwink/common/base/strings';
import { Schemas } from '@hoodwink/common/base/network';
import { isLinux, isWindows, isMacintosh } from '@hoodwink/common/base/platform';
import { isEqual, basename, relativePath } from '@hoodwink/common/base/resources';

export interface IWorkspaceFolderProvider {
  getWorkspaceFolder(resource: URI): { uri: URI; name?: string } | null;
  getWorkspace(): {
    folders: Array<{ uri: URI; name?: string }>;
  };
}
...
export function getPathLabel(
  ...
  rootProvider?: IWorkspaceFolderProvider,
): string {
...
export function tildify(path: string, userHome: string): string {
  ...
  if (
    isLinux ? path.startsWith(normalizedUserHome) : startsWithIgnoreCase(path, normalizedUserHome)
  ) {
