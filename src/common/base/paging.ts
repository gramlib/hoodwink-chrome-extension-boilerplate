...
import { isArray } from '@hoodwink/common/base/types';
...
import { range } from '@hoodwink/common/base/arrays';
...
export function mergePagers<T>(one: IPager<T>, other: IPager<T>): IPager<T> {
  return {
    ...
    getPage(pageIndex: number, token): Promise<T[]> {
      return Promise.all([
        one.getPage(pageIndex, token),
        other.getPage(pageIndex, token),
