...
import { Constants } from '@hoodwink/common/base/uint';
...
function doEqualsIgnoreCase(a: string, b: string, stopAt = a.length): boolean {
  for (let i = 0; i < stopAt; i++) {
    ...
    // Any other charcode
      ...
      String.fromCharCode(codeA).toLowerCase() !== String.fromCharCode(codeB).toLowerCase()
    ) {
      return false;
    }
  }
...
export function breakBetweenGraphemeBreakType(
  breakTypeA: GraphemeBreakType,
  breakTypeB: GraphemeBreakType,
): boolean {
  ...
  // GB13                        [^RI] (RI RI)* RI × RI
  if (
    breakTypeA === GraphemeBreakType.Regional_Indicator &&
