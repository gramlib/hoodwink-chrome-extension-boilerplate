...
export const IInstantiationService = createServiceDecorator<IInstantiationService>(
...
type GetLeadingNonServiceArgs<Args> = Args extends [...BrandedService[]]
  ...
  ? [A1, A2, A3, A4]
  : Args extends [infer A1, infer A2, infer A3, infer A4, infer A5, ...BrandedService[]]
  ? [A1, A2, A3, A4, A5]
  : Args extends [infer A1, infer A2, infer A3, infer A4, infer A5, infer A6, ...BrandedService[]]
  ? [A1, A2, A3, A4, A5, A6]
  : Args extends [
      infer A1,
      infer A2,
...
function storeServiceDependency(
  ...
  if ((target as any)[_util.DI_TARGET] === target) {
    (target as any)[_util.DI_DEPENDENCIES].push({ id, index, optional: optionalFlag });
  } else {
    (target as any)[_util.DI_DEPENDENCIES] = [{ id, index, optional: optionalFlag }];
